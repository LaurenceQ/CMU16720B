...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        ...
        lamb = 1
        #lamb = 0
        g1 = np.linalg.inv(X @ X.T + lamb * np.eye(X.shape[0])) @ X @ Y
        g1 = g1.reshape((29, 45))
        img1 = correlate(img, g1)
        
        g0 = np.linalg.inv(X @ X.T) @ X @ Y
        g0 = g0.reshape((29, 45))
        np.save("g0", g0)
        np.save("g1", g1)
        img0 = correlate(img, g0)
        
        fig, axes = plt.subplots(2, 1)
        axes[0].imshow(img0, cmap = 'gray')
        axes[0].set_title("without regualarization")
        axes[0].axis('off')
        ...
        axes[1].axis('off')
        axes[1].set_title("with regularization")
        fig.suptitle('Result weight factor g', fontsize=16)
        plt.show()
        fig, axes = plt.subplots(2, 1)
        img1 = convolve(img, g1)
        img0 = convolve(img, g0)
        axes[0].imshow(img0, cmap = 'gray')
        axes[0].set_title("without regualarization")
        axes[0].axis('off')
        axes[1].imshow(img1, cmap = 'gray')
