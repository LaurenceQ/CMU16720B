...
'''
    patchsize = np.sqrt(compareX.shape[0])
    Hei = gaussian_pyramid.shape[0]
    Wid = gaussian_pyramid.shape[1]
    locs = []
    desc = []
    X_xaxis, X_yaxis = coordinate_decoder(compareX, patch_width=patchsize) - patchsize // 2
    Y_xaixs, Y_yaxis = coordinate_decoder(compareY, patch_width=patchsize) - patchsize // 2
    for vec in locsDoG:
        if vec[0] < patchsize // 2 or vec[0] + patchsize // 2 >= Hei or vec[1] < patchsize // 2 or vec[1] + patchsize // 2 >= Wid : continue
        locs.append(vec)
        level = levels.index(vec[-1])
        X_xaxis += vec[0]
        Y_xaxis += vec[0]
        X_yaxis += vec[1]
        Y_yaxis += vec[1]
        X_mask = gaussian_pyramid[X_xaxis, X_yaxis, level]
        Y_mask = gaussian_pyramid[Y_xaixs, Y_yaxis, level]
        dictator = X_mask < Y_mask
        desc.append(dictator)
        X_xaxis -= vec[0]
        Y_xaxis -= vec[0]
        X_yaxis -= vec[1]
        Y_yaxis -= vec[1]
...
'''
    ...
    desc = []
    X_xaxis, X_yaxis = coordinate_decoder(compareX, patch_width=patchsize) - patchsize // 2
    Y_xaxis, Y_yaxis = coordinate_decoder(compareY, patch_width=patchsize) - patchsize // 2
    valid_locs = np.logical_and(locsDoG[:,0] < Hei - patchsize // 2, locsDoG[:,0] >= patchsize // 2)
    valid_locs = np.logical_and(valid_locs, np.logical_and(locsDoG[:,1] < Wid - patchsize // 2, locsDoG[:,1] >= patchsize // 2))
    locs = locsDoG[valid_locs]
    X_coord = np.expand_dims(locs[:,0], axis = -1)
    Y_coord = np.expand_dims(locs[:,1], axis = -1)
    X_xaxis = X_coord + X_xaxis[:,0]
    X_yaxis = Y_coord + X_yaxis[:,0]
    Y_xaxis = X_coord + Y_xaxis[:,0]
    Y_yaxis = Y_coord + Y_yaxis[:,0]
    ...
    X_mask = gaussian_pyramid[X_xaxis, X_yaxis, level]
    Y_mask = gaussian_pyramid[Y_xaxis, Y_yaxis, level]
    desc = X_mask < Y_mask
